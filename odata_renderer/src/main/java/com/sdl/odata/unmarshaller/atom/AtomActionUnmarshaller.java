 ...
 */
@Component
public class AtomActionUnmarshaller extends AbstractUnmarshaller {
    ...
    public int score(ODataRequestContext requestContext) {
        if (isRightMethodForUnmarshall(requestContext.getRequest()) &&
            ...
            if (baseScore > 0) {
                actionScore = baseScore + ACTION_OPERATION_SCORE;
            }
            LOG.debug("Match for AtomActionUnmarshaller: {} with score: {}", requestContext.getRequest(), actionScore);
            return actionScore;
        }
